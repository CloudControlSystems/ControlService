...
# get the data from redis (task_initial)
    input_redis = sid.get('Task0toTask20')
    input_redis = json.loads(input_redis)
    
    outputData = TaskProcessFunction(input_redis, recv_list)

    # i = 0
    # for child in children_ips:
        
    #     print("发送第",str(i+1),"个数据")
    #     client_for_child(child, outputData[i])
