...
def TaskProcessFunction(inputDataRedis, inputData):
    ...
    s3 = inputData[4][1]; v3 = np.matrix(inputData[4][2]); u3 = np.matrix(inputData[4][0])

    s1 = diag(s1); s2 = diag(s2); s3 = diag(s3)

    # the body function
    Ahat, Bhat, Chat, Dhat = Task_E(Ui, Yi, l, Oi, Oim1, u1, s1, v1, u2, s2, v2, u3, s3, v3)
    outputData = [Ahat, Bhat, Chat, Dhat]

    return outputData

def Task_E(Ui, Yi, l, Oi, Oim1, u1, s1, v1, u2, s2, v2, u3, s3, v3):
    
    # deal with l
    l = int(l)
    # obtain Gamma and order
    Gamma_i, Gamma_im1, n = compute_Gamma_and_order(u1, s1, v1, u2, s2, v2, u3, s3, v3, l)
...
def merge_svd_result(u1, s1, v1, u2, s2, v2, u3, s3, v3):
    
    u23, s23, v23 = Task_D(u2, s2, v2, u3, s3, v3)
    s23 = diag(s23)
    print("s23: ", type(s23), np.shape(s23))
    u, s, v = Task_D(u23, s23, v23, u1, s1, v1)
    return u, s, v
...
def compute_Gamma_and_order(u1, s1, v1, u2, s2, v2, u3, s3, v3, l):
